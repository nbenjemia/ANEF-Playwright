...
records.forEach((user: any , i: number) => {
  test(`${user.TypeMesure} - ${user.NumeroEtranger}`, async ({ page }) => {
...
// Appel de la fonction Playwright qui retourne le numéro etranger
...
const lignes = csvDataUpdate.split('\n');
   if (lignes.length > i + 1) {
      const colonnes = lignes[i + 1].split(',');
      colonnes[0] = NumeroEtranger;
      lignes[i + 1] = colonnes.join(',');
...
if (user.AjouterNotification === 'Oui' || (user.AjouterNotification=== "Non" && user.AjouterDecision === 'Oui') ) {
